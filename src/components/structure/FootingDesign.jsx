...
export default function FootingDesign({ projectName, activity }) {
  ...
  useEffect(() => {
    ...
    if (!authStatus) {
      console.log('⚠️ User not authenticated in FootingDesign component');
    } else {
      console.log('✅ User authenticated in FootingDesign component');
    }
    
    if (!projectStatus) {
      console.log('⚠️ Project ID not available in FootingDesign component');
    } else {
      console.log('✅ Project ID available in FootingDesign component:', projectId);
    }
  }, [projectId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        columnLoad: parseFloat(formData.axialLoad),  // ✅ Backend expects columnLoad
        momentX: parseFloat(formData.momentX) || 0,
        momentY: parseFloat(formData.momentY) || 0,
        soilSBC: parseFloat(formData.soilBearingCapacity),  // ✅ Backend expects soilSBC
        clearCover: parseFloat(formData.clearCover),
        // Add missing required fields with calculated values
        footingLength: Math.max(parseFloat(formData.columnWidth) / 1000 + 0.5, 1.0),  // Convert mm to m, add 0.5m margin
        footingWidth: Math.max(parseFloat(formData.columnDepth) / 1000 + 0.5, 1.0),   // Convert mm to m, add 0.5m margin
        footingDepth: 0.3,  // Default footing depth in meters
        barDiameter: 12  // Default bar diameter in mm
      };
      ...
      console.log('🌐 Making API call to:', `/api/structure/footing-design`);
      ...
      console.error('❌ Footing design calculation error:', err);
