...
export default function Editor({
  ...
  const floorTransformerRef = useRef(null);
  ...
  // Handle transformer attachment when floor is selected
  useEffect(() => {
    if (
      selectedFloorId &&
      floorTransformerRef.current &&
      floorShapeRefs.current[selectedFloorId]
    ) {
      const node = floorShapeRefs.current[selectedFloorId];
      floorTransformerRef.current.nodes([node]);
      floorTransformerRef.current.getLayer().batchDraw();
    }
  }, [selectedFloorId]);
  ...
  const finishPolygonDrawing = () => {
    ...
    // Use first floor coordinates if available, otherwise use first polygon point
    ...
    // Adjust all points relative to the final position
    const adjustedPoints = closedPoints.map((point) => ({
      x: point.x - firstPoint.x + finalX,
      y: point.y - firstPoint.y + finalY,
    }));
    ...
    if (!firstFloorCoordinates) {
      ...
      console.log("Editor: Stored first floor coordinates from polygon:", {
        x: finalX,
        y: finalY,
      });
    }
    ...
    console.log("Editor: Polygon floor created and selected:", floorShape);
  ...
  const handleFloorDragEnd = (shapeId, x, y) => {
    ...
    if (currentFloor) {
      ...
      const updatedShape = updatedShapes.find((s) => s.id === shapeId);
      if (updatedShape) {
        updateFloorInBackend(shapeId, updatedShape, currentFloor.id).catch(
          (error) => {
  ...
  const handleFloorMouseUp = () => {
    ...
    if (width > 10 && height > 10) {
      ...
      // Use first floor coordinates if available, otherwise use current coordinates
      const finalX = firstFloorCoordinates ? firstFloorCoordinates.x : x;
      const finalY = firstFloorCoordinates ? firstFloorCoordinates.y : y;

      // Create the floor data object (like manual entry)
      const floorData = {
        id: `floor-${Date.now()}`,
        type: "floor",
        shape: "rectangle",
      ...
      console.log(
        "Editor: Floor drawn, showing modal for finalization",
        floorData
      );
      ...
      console.log("Editor: Setting modal data:", modalData);
      setFreehandFloorData(modalData);
    ...
    // Reset drawing state (like room creation) - but don't reset immediately
    // Let the modal handle the state reset when it closes
  };
        ...
        <div className="canvas-viewport">
                  ...
                  !currentFloor && (
                    <>
                      ...
                      {floorDrawingState.polygonPoints.map((point, index) => {
                        const nextPoint =
                          floorDrawingState.polygonPoints[index + 1];
                      ...
                      {floorDrawingState.currentPoint &&
                        floorDrawingState.polygonPoints.length > 0 && (
                          <Line
                            points={[
                              floorDrawingState.polygonPoints[
                                floorDrawingState.polygonPoints.length - 1
                              ].x,
                              floorDrawingState.polygonPoints[
                                floorDrawingState.polygonPoints.length - 1
                              ].y,
                            ...
                            stroke={
                              floorDrawingState.currentPoint ===
                              floorDrawingState.polygonPoints[0]
                                ? "#22c55e"
                                : "#1e40af"
                            }
                            strokeWidth={3}
                      ...
                      {/* Finish button */}
                      {floorDrawingState.showFinishButton &&
                        floorDrawingState.finishButtonPosition && (
                          <Rect
          ...
          {console.log('🎯 Editor: User project floors:', userProjectFloors)} */}
            ...
            />
                ...
                {/* Debug: Show all floors as rectangles */}
                {floors.map((floor, index) => {
                  // Check if floor has shapes and the first shape exists
                  if (!floor.shapes || !floor.shapes[0]) {
                    return null; // Skip rendering if no shapes
                  }
                  ...
                  return (
                    <Rect
                      ...
                      x={300 + index * 150}
                      y={400}
                      width={floor.shapes[0].width || 100}
                      height={floor.shapes[0].height || 100}
            ...
            if (shape.shape === 'rectangle') {
              ...
              const width =100 * shape.width ;
              const height = 100 * shape.height;
              
              console.log('🎯 Editor: Rendering rectangle shape:', {
                id: shape.id,
                x, y, width, height,
            ...
            console.log('🎯 Editor: Rendering polygon shape - points validation:', {
              pointsLength: shape.points?.length,
              points: shape.points,
              hasValidPoints: shape.points && shape.points.length > 0
            });
              // Use the already converted pixel values
              ...
              console.log('🎯 Editor: Rendering polygon shape:', {
                id: shape.id,
                points: shape.points,
            ...
            console.log('🎯 Editor: Shape not rendered - invalid shape data:', {
              shape: shape,
              shapeType: shape.shape,
